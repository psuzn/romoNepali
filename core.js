...
function getAllUnicode(s) {
    ...
    while (tryString.length > 0) {
        ...
        if (u || tryString.length <= 1) {
            if (u) allUnicode += getUnicode(u, !(nextTryString.replace(/^\s+|\s+|\\$/, '').length > 0));
...
this.translate=function translate(input , smart) {
    ...
    if (smart) {
        ...
        while (beginIndex > -1 && endIndex < input.length - 1) {
            ...
            if (beginIndex > -1) {
                ...
                token = input.substring(beginIndex, endIndex + 1);
                mask = "$-" + tokenCount + "-$";
                nonSmartTokens[mask] = translateWords(token.substring(1, token.length - 1));
                input = input.replace(token, mask);
                endIndex = endIndex - token.length + mask.length;
                tokenCount++;
            }
    ...
    if (smart) {
        for (mask in nonSmartTokens) {
            unicode = unicode.replace(translateWords(mask).replace(" ", ""), nonSmartTokens[mask].replace(/\s$/, ""));
        }
    }
    for (mask in engTokens) {
        unicode = unicode.replace(translateWords(mask).replace(" ", ""), engTokens[mask]);
    }
